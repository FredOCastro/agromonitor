...
Melhorias: historico RT clicavel, safras total BR + paises, historico frete por corredor
"""
import json, os
...
# ── Historico relacao de troca (3 safras + atual) ──────────────────────────
HIST_RT = {
...
# ── Historico fretes (R$/ton por corredor, 4 safras) ──────────────────────
HIST_FRETE = {
    "Sorriso MT → Paranagua PR":      [198,218,240,248],
    "Rondonopolis MT → Santos SP":    [174,192,211,218],
    "Rio Verde GO → Santos SP":       [112,124,139,144],
    "Rio Verde GO → Paranagua PR":    [120,133,149,154],
    "Uberlandia MG → Santos SP":      [72, 80, 91, 94],
    "Barreiras BA → Santos SP":       [138,153,172,178],
    "Cascavel PR → Paranagua PR":     [42, 47, 52, 54],
}
...
def rt_card_clicavel(insumo_key, label, valor_atual, nota, cor):
    hist = HIST_RT.get(insumo_key, {})
    soja_hist  = hist.get("soja", []) + [float(valor_atual) if valor_atual != "?" else 0]
    milho_hist = hist.get("milho", []) + [None]
    alg_hist   = hist.get("algodao", []) + [None]
    safras_all = SAFRAS_RT
    soja_js  = json.dumps([round(v,3) if v else None for v in soja_hist])
    milho_js = json.dumps([round(v,3) if v else None for v in milho_hist])
    alg_js   = json.dumps([round(v,3) if v else None for v in alg_hist])
    safras_js = json.dumps(safras_all)
    chart_id = "rtChart_" + insumo_key
    return (
        "<div style=\"border:1px solid #eee;border-radius:8px;padding:12px;cursor:pointer;transition:box-shadow .15s;\" "
        "onclick=\"toggleRtChart('"+chart_id+"')\" "
        "onmouseover=\"this.style.boxShadow='0 2px 8px rgba(0,0,0,0.10)'\" "
        "onmouseout=\"this.style.boxShadow='none'\">"
        "<div style=\"font-size:10px;color:#888;text-transform:uppercase;margin-bottom:4px;\">"+label+"</div>"
        "<div style=\"font-size:22px;font-weight:700;color:"+cor+";\">"+s(valor_atual)+"</div>"
        "<div style=\"font-size:11px;color:#666;margin-top:3px;\">"+nota+"</div>"
        "<div style=\"font-size:10px;color:#888;margin-top:6px;\">&#9660; clique para historico</div>"
        "<div id=\""+chart_id+"\" style=\"display:none;margin-top:12px;\">"
        "<div style=\"position:relative;height:160px;\">"
        "<canvas id=\"canvas_"+chart_id+"\" role=\"img\" aria-label=\"Historico "+label+"\"></canvas>"
        "</div>"
        "<div style=\"display:flex;gap:10px;margin-top:6px;font-size:10px;\">"
        "<span style=\"display:flex;align-items:center;gap:3px;\"><span style=\"width:8px;height:8px;background:#1D9E75;border-radius:2px;display:inline-block;\"></span>Soja</span>"
        "<span style=\"display:flex;align-items:center;gap:3px;\"><span style=\"width:8px;height:8px;background:#BA7517;border-radius:2px;display:inline-block;\"></span>Milho</span>"
        "<span style=\"display:flex;align-items:center;gap:3px;\"><span style=\"width:8px;height:8px;background:#7F77DD;border-radius:2px;display:inline-block;\"></span>Algodao</span>"
        "</div>"
        "<div style=\"font-size:10px;color:#aaa;margin-top:4px;\">sc ou @ por unidade &#183; * safra atual</div>"
        "</div>"
        "<script>"
        "document.addEventListener('DOMContentLoaded',function(){"
        "var ctx=document.getElementById('canvas_"+chart_id+"');"
        "if(ctx&&!ctx._chartCreated){ctx._chartCreated=true;"
        "new Chart(ctx,{type:'bar',"
        "data:{labels:"+safras_js+","
        "datasets:["
        "{label:'Soja',data:"+soja_js+",backgroundColor:['#9FE1CB','#9FE1CB','#9FE1CB','#1D9E75']},"
        "{label:'Milho',data:"+milho_js+",backgroundColor:['#FAC775','#FAC775','#FAC775','#BA7517']},"
        "{label:'Algodao',data:"+alg_js+",backgroundColor:['#AFA9EC','#AFA9EC','#AFA9EC','#7F77DD']}"
        "]},"
        "options:{responsive:true,maintainAspectRatio:false,"
        "plugins:{legend:{display:false}},"
        "scales:{x:{ticks:{color:'#888',font:{size:10}},grid:{color:'rgba(0,0,0,0.04)'}},"
...
def frete_row_clicavel(rota_key, origem, destino, modal, valor, var_pct, ref_pga):
    hist = HIST_FRETE.get(rota_key, [])
    pct_soja = round(valor / float(ref_pga) / 16.67 * 100, 1)
    vc = "color:#A32D2D;" if var_pct > 3 else "color:#BA7517;"
    chart_id = "freteChart_" + rota_key.replace(" ","_").replace("→","para").replace("/","_")
    safras_js = json.dumps(SAFRAS_FRETE)
    hist_js   = json.dumps(hist)
    max_v = max(hist) if hist else valor
    cores_barra = []
    for i, v in enumerate(hist):
        if i == len(hist)-1: cores_barra.append("#1D9E75")
        elif v == max_v: cores_barra.append("#E24B4A")
        else: cores_barra.append("#9FE1CB")
    cores_js = json.dumps(cores_barra)

    return (
        "<tr onclick=\"toggleFreteChart('"+chart_id+"')\" style=\"cursor:pointer;\" "
        "onmouseover=\"this.style.background='#f8fff4'\" onmouseout=\"this.style.background=''\">"
        "<td style=\"font-size:12px;font-weight:500;\">"+origem+" &#8594; "+destino
        +"<br><span style=\"font-size:10px;color:#888;\">"+modal+"</span>"
        +"<span style=\"font-size:10px;color:#888;\"> &#183; &#9660; clique para historico</span></td>"
        "<td style=\"text-align:right;font-size:14px;font-weight:700;\">R$ "+str(valor)+"/ton</td>"
        "<td style=\"text-align:right;font-size:11px;"+vc+"\">&#9650; +"+s(var_pct,1)+"%</td>"
        "<td style=\"text-align:right;font-size:11px;color:#888;\">"+str(pct_soja)+"% soja</td>"
        "</tr>"
        "<tr id=\""+chart_id+"\" style=\"display:none;\">"
        "<td colspan=\"4\" style=\"padding:12px 16px;background:#f8fff4;\">"
        "<div style=\"font-size:12px;font-weight:600;color:#1a3a1a;margin-bottom:8px;\">Historico: "+origem+" &#8594; "+destino+" (R$/ton)</div>"
        "<div style=\"position:relative;height:140px;\">"
        "<canvas id=\"canvas_"+chart_id+"\" role=\"img\" aria-label=\"Historico frete "+origem+"\"></canvas>"
        "</div>"
        "<div style=\"font-size:10px;color:#aaa;margin-top:4px;\">&#128994; = safra atual &#183; &#128308; = pico historico</div>"
        "<script>"
        "document.addEventListener('DOMContentLoaded',function(){"
        "var ctx=document.getElementById('canvas_"+chart_id+"');"
        "if(ctx&&!ctx._chartCreated){ctx._chartCreated=true;"
        "new Chart(ctx,{type:'bar',"
        "data:{labels:"+safras_js+","
        "datasets:[{label:'R$/ton',data:"+hist_js+",backgroundColor:"+cores_js+"}]},"
        "options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},"
        "scales:{x:{ticks:{color:'#888',font:{size:11}},grid:{color:'rgba(0,0,0,0.04)'}},"
        "y:{ticks:{color:'#888',font:{size:11},callback:v=>'R$'+v},grid:{color:'rgba(0,0,0,0.04)'},"
        "suggestedMin:"+str(int(min(hist)*0.85) if hist else 0)+"}}}"
        "});}})"
        "</script>"
        "</td></tr>"
    )
...
def tabela_mundial_completa(data, unidade):
    """Tabela com historico 3 safras + projecao + grafico de barras embutido."""
    h = "<table class=\"tbl\"><thead><tr><th>Pais</th><th style=\"text-align:right;\">23/24</th><th style=\"text-align:right;\">24/25</th><th style=\"text-align:right;\">25/26</th><th style=\"text-align:right;\">Proj.26/27*</th><th style=\"text-align:right;\">Tendencia</th></tr></thead><tbody>"
    total = {"prod_2324":0,"prod_2425":0,"prod_2526":0,"proj_2627":0}
    for pais, v in data.items():
        if pais == "Mundo": continue
        vc = "color:#27500A;" if v.get("proj_2627",0)>=v.get("prod_2526",0) else "color:#A32D2D;"
        seta = "&#9650;" if v.get("proj_2627",0)>=v.get("prod_2526",0) else "&#9660;"
        for k in total: total[k] += v.get(k,0)
        # mini sparkline via largura de barra proporcional
        max_v = max(v.get("prod_2324",0), v.get("prod_2425",0), v.get("prod_2526",0), v.get("proj_2627",0), 1)
        bar_w = round(v.get("prod_2526",0)/max_v*60) if max_v else 0
        bar_w2 = round(v.get("proj_2627",0)/max_v*60) if max_v else 0
        spark = "<div style=\"display:flex;flex-direction:column;gap:2px;\"><div style=\"height:5px;width:"+str(bar_w)+"px;background:#1D9E75;border-radius:2px;\"></div><div style=\"height:5px;width:"+str(bar_w2)+"px;background:#FAEEDA;border:1px solid #BA7517;border-radius:2px;\"></div></div>"
    ...
    # Linha total
    vc_t = "color:#27500A;" if total["proj_2627"]>=total["prod_2526"] else "color:#A32D2D;"
    h += "<tr style=\"background:#f8fff4;font-weight:700;\"><td>&#127482;&#127480; Total mundo</td><td style=\"text-align:right;\">"+s(total["prod_2324"],0)+"</td><td style=\"text-align:right;\">"+s(total["prod_2425"],0)+"</td><td style=\"text-align:right;\">"+s(total["prod_2526"],0)+"</td><td style=\"text-align:right;"+vc_t+"font-style:italic;\">"+s(total["proj_2627"],0)+"*</td><td><div style=\"font-size:10px;color:#666;\">"+unidade+"</div></td></tr>"
    h += "</tbody></table>"
    return h
...
def painel_enso(oni, status, tend, enso_res):
    ...
    return (
        ...
        "<span>-2.0</span><span>-1.5</span><span>-1.0</span><span>-0.5</span><span>0</span><span>+0.5</span><span>+1.5</span><span>+2.0</span><span>+2.5</span></div>"
        ...
        "<div style=\"font-size:11px;font-weight:500;color:#888;text-transform:uppercase;letter-spacing:.05em;margin-bottom:10px;\">Impacto por cenario &#8212; Centro-Oeste &amp; Triangulo Mineiro</div>"
        "<div style=\"display:grid;grid-template-columns:repeat(5,1fr);gap:8px;\">"
        "<div style=\"background:#E6F1FB;border:0.5px solid #85B7EB;border-radius:8px;padding:10px 8px;text-align:center;\"><div style=\"font-size:11px;font-weight:600;color:#0C447C;margin-bottom:4px;\">La Nina</div><div style=\"font-size:18px;font-weight:700;color:#0C447C;\">"+str(p_la)+"%</div><div style=\"font-size:10px;color:#185FA5;\">probabilidade</div><div style=\"font-size:10px;color:#27500A;margin-top:6px;\">Soja: +5/+10%%</div><div style=\"font-size:10px;color:#27500A;\">Milho: +5/+8%%</div><div style=\"font-size:10px;color:#27500A;\">Algodao: +3/+6%%</div></div>"
        "<div style=\"background:#f5f5f3;border:0.5px solid #B4B2A9;border-radius:8px;padding:10px 8px;text-align:center;\"><div style=\"font-size:11px;font-weight:600;color:#444;margin-bottom:4px;\">Neutro</div><div style=\"font-size:18px;font-weight:700;color:#444;\">"+str(probs["Neutro"])+"%</div><div style=\"font-size:10px;color:#666;\">probabilidade</div><div style=\"font-size:10px;color:#444;margin-top:6px;\">Soja: 0/+5%%</div><div style=\"font-size:10px;color:#444;\">Milho: 0/+5%%</div><div style=\"font-size:10px;color:#444;\">Algodao: 0/+3%%</div></div>"
        "<div style=\"background:#FAEEDA;border:0.5px solid #EF9F27;border-radius:8px;padding:10px 8px;text-align:center;\"><div style=\"font-size:11px;font-weight:600;color:#854F0B;margin-bottom:4px;\">El Nino Fraco</div><div style=\"font-size:18px;font-weight:700;color:#854F0B;\">"+str(probs["El Nino Fraco"])+"%</div><div style=\"font-size:10px;color:#BA7517;\">probabilidade</div><div style=\"font-size:10px;color:#A32D2D;margin-top:6px;\">Soja: -5/-15%%</div><div style=\"font-size:10px;color:#A32D2D;\">Milho: -10/-20%%</div><div style=\"font-size:10px;color:#A32D2D;\">Algodao: -5/-10%%</div></div>"
        "<div style=\"background:#FCEBEB;border:0.5px solid #E24B4A;border-radius:8px;padding:10px 8px;text-align:center;\"><div style=\"font-size:11px;font-weight:600;color:#A32D2D;margin-bottom:4px;\">El Nino Moder.</div><div style=\"font-size:18px;font-weight:700;color:#A32D2D;\">"+str(probs["El Nino Moderado"])+"%</div><div style=\"font-size:10px;color:#A32D2D;\">probabilidade</div><div style=\"font-size:10px;color:#A32D2D;margin-top:6px;\">Soja: -15/-25%%</div><div style=\"font-size:10px;color:#A32D2D;\">Milho: -20/-30%%</div><div style=\"font-size:10px;color:#A32D2D;\">Algodao: -10/-18%%</div></div>"
        "<div style=\"background:#FCEBEB;border:2px solid #791F1F;border-radius:8px;padding:10px 8px;text-align:center;\"><div style=\"font-size:11px;font-weight:600;color:#791F1F;margin-bottom:4px;\">Super El Nino</div><div style=\"font-size:18px;font-weight:700;color:#791F1F;\">"+str(probs["El Nino Forte Super"])+"%</div><div style=\"font-size:10px;color:#791F1F;\">probabilidade</div><div style=\"font-size:10px;color:#791F1F;margin-top:6px;\">Soja: -25/-35%%</div><div style=\"font-size:10px;color:#791F1F;\">Milho: -30/-40%%</div><div style=\"font-size:10px;color:#791F1F;\">Algodao: -18/-25%%</div></div>"
        "</div>"
        "<div style=\"font-size:10px;color:#aaa;margin-top:8px;\">Impacto na produtividade vs media historica &#183; Episodios 1997/98, 2009/10, 2015/16 e 2023/24</div>"
        "<div style=\"height:0.5px;background:#eee;margin:16px 0;\"></div>"
...
def gerar(analise, dados):
    ...
    # Atualiza historico RT com valor atual
    for key, crops in HIST_RT.items():
        ...
        rt_key = rt_map.get(key,"")
        if rt_key:
            crops["soja"][-1]    = round(float(rt_soja.get(rt_key, crops["soja"][-2] or 0)), 3)
            crops["milho"][-1]   = round(float(rt_milho.get(rt_key.replace("sc_","sc_"), crops["milho"][-2] or 0)), 3) if "ureia" in key or "map" in key or "kcl" in key else crops["milho"][-2]
            crops["algodao"][-1] = round(float(rt_alg.get(rt_key.replace("sc_ton","arr_ton").replace("sc_l","arr_l"), crops["algodao"][-2] or 0)), 3)
...
@media(max-width:480px){.g4{grid-template-columns:1fr 1fr;}}"""
    ...
    # Algodao BR
    html += "<div id=\"sr-algodao\" style=\"display:none;\"><div class=\"g4\" style=\"margin-bottom:12px;\"><div class=\"met\"><div class=\"ml\">Area total BR</div><div class=\"mv\">"+s(alg_br.get("area_mha","?"),2)+" Mha</div></div><div class=\"met\"><div class=\"ml\">Producao total BR</div><div class=\"mv\" style=\"color:#534AB7;\">"+s(alg_br.get("producao_mfardos","?"),1)+" Mf</div><div class=\"ms\" style=\"color:#27500A;\">&#9650; Record</div></div><div class=\"met\"><div class=\"ml\">Produtividade</div><div class=\"mv\">"+str(alg_br.get("produtividade_arrha","?"))+" @/ha</div></div><div class=\"met\"><div class=\"ml\">Colheita</div><div class=\"mv\" style=\"color:#1D9E75;\">"+str(alg_br.get("colheita_pct","?"))+"%</div></div></div>"
    html += "<table class=\"tbl\"><thead><tr><th>Estado</th><th style=\"text-align:right;\">Area(Mha)</th><th style=\"text-align:right;\">Prod.(Mf)</th><th style=\"text-align:right;\">@/ha</th><th style=\"text-align:right;\">Colheita</th><th style=\"text-align:right;\">vs 24/25</th></tr></thead><tbody>"+tabela_estado_algodao(alg_br.get("por_estado",{}))+"</tbody></table></div>"

    # Mundial com total e sparkline
    html += "<div id=\"sr-ms\" style=\"display:none;\"><div style=\"font-size:11px;color:#888;margin-bottom:8px;\">Milhoes de Toneladas (Mt) &#183; USDA WASDE &#183; Mini-barra: &#9632; 25/26 &#183; &#9633; proj.26/27*</div>"+tabela_mundial_completa(mundial.get("soja_mt",{}), "Mt")+"</div>"
    html += "<div id=\"sr-mm\" style=\"display:none;\"><div style=\"font-size:11px;color:#888;margin-bottom:8px;\">Milhoes de Toneladas (Mt) &#183; USDA WASDE</div>"+tabela_mundial_completa(mundial.get("milho_mt",{}), "Mt")+"</div>"
    html += "<div id=\"sr-ma\" style=\"display:none;\"><div style=\"font-size:11px;color:#888;margin-bottom:8px;\">Milhoes de Fardos &#183; USDA WASDE</div>"+tabela_mundial_completa(mundial.get("algodao_mf",{}), "Mf")+"</div>"
    html += "<div style=\"font-size:10px;color:#aaa;margin-top:8px;\">* Projecao preliminar &#183; "+safras.get("referencia","CONAB")+"</div></div>"
    ...
    html += "<div style=\"font-size:12px;color:#666;margin-bottom:10px;\">Reajuste ANTT: +"+s(frete.get("reajuste_antt_pct",3.2),1)+"% &#183; Diesel: R$ "+s(frete.get("diesel_rl",6.18))+"/L &#183; <span style=\"color:#27500A;\">&#9660; clique em uma rota para ver o historico</span></div>"
    ...
    for r in frete.get("rotas",[]):
        rota_key = r["origem"] + " → " + r["destino"]
        html += frete_row_clicavel(rota_key, r["origem"], r["destino"], r["modal"], r["valor"], r.get("var_pct",3.0), ref_pga)
    ...
    # ── RELACAO DE TROCA com historico clicavel ──────────────────────────────
    html += "<div id=\"tab-insumos\" class=\"tc\"><div class=\"wrap\">"
    html += "<div class=\"card\"><div class=\"ct\">Relacao de Troca &#8212; Soja por Insumo &#183; Paranagua R$ "+s(ref_pga)+"/sc</div>"
    html += "<div style=\"font-size:12px;color:#27500A;margin-bottom:12px;\">&#9660; Clique em qualquer card para ver o historico comparativo das ultimas 3 safras</div>"
    html += "<div class=\"g3\">"
    html += rt_card_clicavel("ureia","Ureia (soja/ton)",rt_soja.get("ureia_sc_ton","?"),"R$ "+s(insumos.get("ureia_ton",336),0)+"/ton","#A32D2D")
    html += rt_card_clicavel("map","MAP (soja/ton)",rt_soja.get("map_sc_ton","?"),"R$ "+s(insumos.get("map_ton",423),0)+"/ton","#BA7517")
    html += rt_card_clicavel("kcl","KCl (soja/ton)",rt_soja.get("kcl_sc_ton","?"),"R$ "+s(insumos.get("kcl_ton",247),0)+"/ton","#27500A")
    html += rt_card_clicavel("glifosato","Glifosato (soja/L)",rt_soja.get("glifosato_sc_l","?"),"R$ "+s(insumos.get("glifosato_l",18.9),2)+"/L","#27500A")
    html += rt_card_clicavel("diesel","Diesel (soja/L)",rt_soja.get("diesel_sc_l","?"),"R$ "+s(insumos.get("diesel_l",6.18),2)+"/L","#BA7517")
    html += "<div style=\"border:1px solid #eee;border-radius:8px;padding:12px;\"><div style=\"font-size:10px;color:#888;text-transform:uppercase;margin-bottom:4px;\">Como interpretar</div><div style=\"font-size:12px;color:#555;line-height:1.7;\">Quantas sacas de soja equivalem a 1 unidade do insumo. <strong>Quanto menor, melhor</strong> para o produtor &#8212; significa que o insumo ficou mais barato em relacao a soja.</div><div style=\"font-size:11px;color:#888;margin-top:8px;\">Media historica ureia: 2.1 sc/ton &#183; Atual: "+s(rt_soja.get("ureia_sc_ton","?"))+" sc/ton</div></div>"
    html += "</div></div>"
    html += "<div class=\"card\"><div class=\"ct\">Custo de Producao &#8212; Soja Safra 26/27 (R$/ha)</div><table class=\"tbl\"><tr><th>Componente</th><th style=\"text-align:right;\">MT</th><th style=\"text-align:right;\">GO</th><th style=\"text-align:right;\">PR</th><th style=\"text-align:right;\">MG Tri.</th></tr><tr><td>Fertilizantes</td><td style=\"text-align:right;\">1.420</td><td style=\"text-align:right;\">1.380</td><td style=\"text-align:right;\">1.280</td><td style=\"text-align:right;\">1.350</td></tr><tr><td>Defensivos</td><td style=\"text-align:right;\">980</td><td style=\"text-align:right;\">920</td><td style=\"text-align:right;\">860</td><td style=\"text-align:right;\">900</td></tr><tr><td>Sementes</td><td style=\"text-align:right;\">420</td><td style=\"text-align:right;\">410</td><td style=\"text-align:right;\">390</td><td style=\"text-align:right;\">400</td></tr><tr><td>Operacoes/Maq.</td><td style=\"text-align:right;\">680</td><td style=\"text-align:right;\">640</td><td style=\"text-align:right;\">580</td><td style=\"text-align:right;\">620</td></tr><tr><td>Frete interno</td><td style=\"text-align:right;\">340</td><td style=\"text-align:right;\">220</td><td style=\"text-align:right;\">90</td><td style=\"text-align:right;\">160</td></tr><tr><td>Outros</td><td style=\"text-align:right;\">580</td><td style=\"text-align:right;\">540</td><td style=\"text-align:right;\">490</td><td style=\"text-align:right;\">520</td></tr><tr style=\"background:#f8fff4;\"><td><b>TOTAL</b></td><td style=\"text-align:right;\"><b>4.420</b></td><td style=\"text-align:right;\"><b>4.110</b></td><td style=\"text-align:right;\"><b>3.690</b></td><td style=\"text-align:right;\"><b>3.950</b></td></tr><tr style=\"background:#fff8e6;\"><td><i>Break-even sc/ha</i></td><td style=\"text-align:right;color:#A32D2D;\"><b>"+s(4420/float(ref_pga),1)+"</b></td><td style=\"text-align:right;color:#BA7517;\"><b>"+s(4110/float(ref_pga),1)+"</b></td><td style=\"text-align:right;color:#27500A;\"><b>"+s(3690/float(ref_pga),1)+"</b></td><td style=\"text-align:right;color:#BA7517;\"><b>"+s(3950/float(ref_pga),1)+"</b></td></tr></table><div style=\"font-size:10px;color:#aaa;margin-top:6px;\">Break-even calculado com preco Paranagua R$ "+s(ref_pga)+"/sc &#183; Nao inclui arrendamento</div></div>"
    html += "</div></div>"
    ...
    html += "<div class=\"card\"><div class=\"ct\">Relacao de Troca &#8212; Algodao</div><div class=\"g3\">"
    html += rt_card_clicavel("ureia","Ureia (@/ton)",rt_alg.get("ureia_arr_ton","?"),"R$ "+s(insumos.get("ureia_ton",336),0)+"/ton","#BA7517")
    html += rt_card_clicavel("map","MAP (@/ton)",rt_alg.get("map_arr_ton","?"),"R$ "+s(insumos.get("map_ton",423),0)+"/ton","#BA7517")
    html += rt_card_clicavel("kcl","KCl (@/ton)",rt_alg.get("kcl_arr_ton","?"),"R$ "+s(insumos.get("kcl_ton",247),0)+"/ton","#27500A")
    html += "</div></div>"
    html += "</div></div>"
    ...
    html += "<script>"
    html += "function st(n,el){document.querySelectorAll('.tc').forEach(e=>e.classList.remove('active'));document.querySelectorAll('.tab').forEach(e=>e.classList.remove('active'));document.getElementById('tab-'+n).classList.add('active');el.classList.add('active');}"
    html += "function sr(n,el){['soja','milho','algodao','ms','mm','ma'].forEach(c=>{var e=document.getElementById('sr-'+c);if(e)e.style.display=c===n?'':'none';});document.querySelectorAll('.rtab').forEach(b=>b.classList.remove('active'));el.classList.add('active');}"
    html += "function toggleRtChart(id){var el=document.getElementById(id);if(el){el.style.display=el.style.display==='none'?'block':'none';}}"
    html += "function toggleFreteChart(id){var el=document.getElementById(id);if(el){el.style.display=el.style.display==='none'?'table-row':'none';}}"
    html += "new Chart(document.getElementById('cSH'),{type:'line',data:{labels:"+hl(hist_soja)+",datasets:[{label:'Soja CBOT',data:"+hds(hist_soja)+",borderColor:'#1D9E75',backgroundColor:'rgba(29,158,117,0.07)',fill:true,tension:0.3,pointRadius:3}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{ticks:{color:'"+tc2+"',font:{size:10}},grid:{color:'"+gc+"'}},y:{ticks:{color:'"+tc2+"',callback:v=>'$'+v.toFixed(2)},grid:{color:'"+gc+"'}}}}});"
    html += "new Chart(document.getElementById('cMH'),{type:'line',data:{labels:"+hl(hist_milho)+",datasets:[{label:'Milho CBOT',data:"+hdm(hist_milho)+",borderColor:'#BA7517',backgroundColor:'rgba(186,117,23,0.07)',fill:true,tension:0.3,pointRadius:3}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{ticks:{color:'"+tc2+"',font:{size:10}},grid:{color:'"+gc+"'}},y:{ticks:{color:'"+tc2+"',callback:v=>'$'+v.toFixed(2)},grid:{color:'"+gc+"'}}}}});"
    html += "new Chart(document.getElementById('cAH'),{type:'line',data:{labels:"+hl(hist_alg)+",datasets:[{label:'Algodao ICE',data:"+hda(hist_alg)+",borderColor:'#7F77DD',backgroundColor:'rgba(127,119,221,0.07)',fill:true,tension:0.3,pointRadius:3}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{ticks:{color:'"+tc2+"',font:{size:10}},grid:{color:'"+gc+"'}},y:{ticks:{color:'"+tc2+"',callback:v=>'$'+v.toFixed(4)},grid:{color:'"+gc+"'}}}}});"
    html += "new Chart(document.getElementById('cSF'),{type:'line',data:{labels:"+sfl+",datasets:[{label:'Soja futuros',data:"+sfd+",borderColor:'#1D9E75',backgroundColor:'rgba(29,158,117,0.08)',fill:true,tension:0.3,pointRadius:5,pointBackgroundColor:'#1D9E75'}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{ticks:{color:'"+tc2+"'},grid:{color:'"+gc+"'}},y:{ticks:{color:'"+tc2+"',callback:v=>'$'+v.toFixed(2)},grid:{color:'"+gc+"'}}}}});"
    html += "new Chart(document.getElementById('cFrete'),{type:'bar',data:{labels:['Cascavel PR','Uberlandia MG','Rio Verde GO','Rondonopolis MT','Sorriso MT','Barreiras BA'],datasets:[{label:'Frete %% soja',data:[4.0,6.9,10.6,16.1,18.3,13.1],backgroundColor:['#9FE1CB','#9FE1CB','#FAC775','#E24B4A','#E24B4A','#EF9F27']}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{ticks:{color:'"+tc2+"',font:{size:10}},grid:{color:'"+gc+"'}},y:{ticks:{color:'"+tc2+"',callback:v=>v+'%%'},grid:{color:'"+gc+"'}}}}});"
    html += "</script></body></html>"
    return html

def main():
    print("\nGerando dashboard v3 com historicos RT, safras e frete...")
